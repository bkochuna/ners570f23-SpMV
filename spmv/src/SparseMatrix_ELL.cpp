...
namespace SpMV
{
    ...
    fp_type SparseMatrix_ELL<fp_type>::getCoefficient(const size_t row,const size_t col){
        ...
        for (size_t i=0;i<this->_nrowsmax;i++){
            if(this->colIdx[this->_nrows*i+row]==col){
                output = this->val[this->_nrows*i+row];
            }
        }
        return output;
    }
    ...
    void SparseMatrix_ELL<fp_type>::matvec(fp_type *vecin, fp_type *vecout){
        // I have added some random code here, person assigned ell matvec should put code here
        vecin++;
        vecout++;
    }
